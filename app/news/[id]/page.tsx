...
async function fetchNews(id: string): Promise<NewsForPage | null> {
  try {
    const res = await fetch(`${API_URL}/api/news/${id}`, { next: { revalidate: 60 } });
...
async function fetchRelated(category: string, excludeId: string): Promise<NewsForPage[]> {
  try {
    const qs = new URLSearchParams({ category, limit: '5' });
    const res = await fetch(`${API_URL}/api/news?${qs}`, { next: { revalidate: 60 } });
    if (!res.ok) return [];
