        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
            {news.type === 'announcement' ? (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.3rem 0.85rem', background: '#fffbeb', color: '#b45309', border: '1px solid #fde68a', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 700 }}>
                📢 Pengumuman
              </span>
            ) : (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.3rem 0.85rem', background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 700 }}>
                📰 Berita
              </span>
            )}
            <span style={{ padding: '0.3rem 0.85rem', background: catColor(news.category) + '12', color: catColor(news.category), border: `1px solid ${catColor(news.category)}30`, borderRadius: '999px', fontSize: '0.75rem', fontWeight: 600 }}>
              {news.category}
            </span>
            {news.pinned && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.3rem 0.85rem', background: '#fef3c7', color: '#d97706', border: '1px solid #fde68a', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 700 }}>
                📌 Disematkan
              </span>
            )}
            {news.tag && (
              <span style={{ padding: '0.3rem 0.85rem', background: '#fff7ed', color: '#d97706', border: '1px solid #fed7aa', borderRadius: '999px', fontSize: '0.75rem', fontWeight: 700 }}>
                {news.tag}
              </span>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.65rem', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
            {[
              ...
              { icon: '👁', text: `${news.views.toLocaleString('id-ID')} dilihat` },
            ].map(m => (
      ...
      <section style={{ background: 'var(--bg-muted)', padding: '2.5rem 1.5rem 6rem' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gap: '2rem' }} className="news-detail-layout">
          ...
          <article>
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '20px', padding: '2.5rem' }}>
              {news.content ? (
                ...
                <p style={{ color: '#94a3b8', fontStyle: 'italic', textAlign: 'center', padding: '3rem 0' }}>
            ...
            <div style={{
              ...
              <a
                ...
                style={{
                  ...
                  borderRadius: '999px', textDecoration: 'none', fontSize: '0.8rem', fontWeight: 700,
                }}
              >
          ...
          <aside>
            {/* Info card */}
            <div style={{
              background: news.type === 'announcement'
              ...
              <h3 style={{ fontSize: '1rem', fontWeight: 800, marginBottom: '0.5rem', lineHeight: 1.3 }}>
                {news.type === 'announcement' ? 'Pengumuman Resmi' : 'Berita Kampus'}
              </h3>
              <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.72)', lineHeight: 1.6, marginBottom: '1.25rem' }}>
                {news.type === 'announcement'
                  ? 'Informasi ini merupakan pengumuman resmi dari STIA Abdul Haris Makassar.'
                  : 'Temukan informasi dan liputan kegiatan kampus STIA Abdul Haris Makassar.'}
              </p>

              {/* Meta details */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', padding: '1rem', background: 'rgba(255,255,255,0.1)', borderRadius: '12px', marginBottom: '1.25rem' }}>
                {[
                  { label: 'Kategori', value: news.category },
                  { label: 'Tanggal', value: fmtDateShort(news.createdAt) },
                  { label: 'Dilihat', value: `${news.views.toLocaleString('id-ID')} kali` },
                ].map(item => (
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.55)', fontWeight: 600 }}>{item.label}</span>
                    <span style={{ fontSize: '0.78rem', color: 'white', fontWeight: 700 }}>{item.value}</span>
                  </div>
                ))}
                {news.tag && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.55)', fontWeight: 600 }}>Tag</span>
                    <span style={{ fontSize: '0.72rem', padding: '0.2rem 0.55rem', background: 'rgba(245,166,35,0.3)', color: '#fbbf24', borderRadius: '999px', fontWeight: 700 }}>{news.tag}</span>
                  </div>
                )}
              </div>
              ...
              {related.length > 0 && (
                <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.15)' }}>
                  <h4 style={{ fontSize: '0.82rem', fontWeight: 800, color: 'rgba(255,255,255,0.85)', marginBottom: '0.85rem' }}>
                    Konten Terkait
                  </h4>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                    {related.map(item => (
                      <a key={item.id} href={`/p/news/${item.id}`} style={{ textDecoration: 'none' }}>
                        <div
                          style={{
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.3rem' }}>
                            <span style={{ fontSize: '0.65rem' }}>{item.type === 'announcement' ? '📢' : '📰'}</span>
                            <span style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>{item.category}</span>
                          </div>
                          <p style={{ fontSize: '0.78rem', fontWeight: 600, color: 'white', lineHeight: 1.4, margin: 0, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                            {item.title}
                          </p>
                          <p style={{ fontSize: '0.65rem', color: 'rgba(255,255,255,0.45)', margin: '0.25rem 0 0' }}>
