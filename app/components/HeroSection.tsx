...
export default function HeroSection() {
  ...
  return (
    <section id="hero" ref={ref} style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', overflow: 'hidden' }}>
      ...
      {/* Blobs */}
      <div style={{ position: 'absolute', top: '10%', right: '5%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(245,166,35,0.15) 0%, transparent 70%)', borderRadius: '50%', zIndex: 1, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '20%', left: '-5%', width: '350px', height: '350px', background: 'radial-gradient(circle, rgba(26,74,173,0.2) 0%, transparent 70%)', borderRadius: '50%', zIndex: 1, pointerEvents: 'none' }} />

      {/* Main Content */}
      <div style={{ position: 'relative', zIndex: 2, maxWidth: '1280px', margin: '0 auto', width: '100%', padding: '0 1.5rem', paddingTop: '9rem', paddingBottom: '6rem' }}>
        <div style={{ maxWidth: '760px' }}>
          {/* Badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 1rem', background: 'rgba(245,166,35,0.18)', border: '1px solid rgba(245,166,35,0.35)', borderRadius: '999px', marginBottom: '1.5rem', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.7s ease 0.1s' }}>
            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#f5a623', display: 'inline-block' }} />
            <span style={{ color: '#fbbf24', fontWeight: 600, fontSize: '0.8rem', letterSpacing: '0.06em' }}>{t.hero.badge}</span>
          </div>
          ...
          {/* Subtitle */}
          <p style={{ fontSize: 'clamp(1rem, 2vw, 1.2rem)', color: 'rgba(255,255,255,0.82)', lineHeight: 1.75, marginBottom: '2.5rem', maxWidth: '580px', fontWeight: 400, opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(24px)', transition: 'all 0.7s ease 0.35s' }}
            dangerouslySetInnerHTML={{ __html: t.hero.subtitle }}
          />

          {/* CTA Buttons */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.7s ease 0.5s' }}>
            <a href="#daftar" onClick={e => { e.preventDefault(); handleScroll('daftar'); }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.9rem 2rem', background: 'linear-gradient(135deg, #f5a623 0%, #fbbf24 100%)', color: '#0f2d6b', fontWeight: 700, fontSize: '1rem', borderRadius: '999px', textDecoration: 'none', boxShadow: '0 8px 32px rgba(245,166,35,0.45)', transition: 'all 0.25s ease', whiteSpace: 'nowrap' }}
              onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(-3px)'; (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 14px 40px rgba(245,166,35,0.55)'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLAnchorElement).style.boxShadow = '0 8px 32px rgba(245,166,35,0.45)'; }}
            >
              {t.hero.cta1}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1px', background: 'rgba(255,255,255,0.12)', borderRadius: '16px', overflow: 'hidden', marginTop: '4rem', border: '1px solid rgba(255,255,255,0.12)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(32px)', transition: 'all 0.7s ease 0.75s' }} className="hero-stats-grid">
          {t.hero.stats.map(stat => (
            <div key={stat.label} style={{ padding: '1.5rem', background: 'rgba(255,255,255,0.06)', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.72)', fontWeight: 400 }}>{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Scroll indicator */}
      <div style={{ position: 'absolute', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', color: 'rgba(255,255,255,0.5)', fontSize: '0.7rem', letterSpacing: '0.1em', opacity: visible ? 1 : 0, transition: 'opacity 1s ease 1.2s' }}>
        <span>SCROLL</span>
